...
if __name__ == "__main__":
    ...
    # --- Datasets ---
    train_dataset = SplitAudioFolderDataset(coughvid_dir, target_sr=16000, split_type='train')
    eval_dataset = SplitAudioFolderDataset(coughvid_dir, target_sr=16000, split_type='eval')
    ...
    for i, label in enumerate(test_dataset.labels):
        if label not in label_to_idx.values():  # usar values() porque labels já são índices
    ...
    model = PodFineTunedClassifier(
        ...
        panns_weights_path='/workspace/checkpoints/Cnn10_mAP=0.380.pth',
        freeze_panns=True,
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if eval_acc > best_eval_acc:
            ...
            torch.save({'model': best_model_state, 
                       ...
                       "pod_finetuned_classifier_best_cxc_cnn10.pth")
        else:
            patience_counter += 1
    ...
    # --- Evaluation on Coswara test set ---
    model.eval()
